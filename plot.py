...
def plot_mesh_point_station(points, tbs, save=''):
    ...
    plt.xlim(-Parameters.Mesh_rad*1.1, Parameters.Mesh_rad*1.1)
    plt.ylim(-Parameters.Mesh_rad*1.1, Parameters.Mesh_rad*1.1)
...
def plot_received_signal_from_HAPS(W: np.array, HAPS_channels, points, HAPS_users, Null_points, user=-1, save=''):
    ...
    fig= plt.figure(figsize=(8, 8))
    ax = plt.subplot()
    sc = plt.scatter(x, y, c=value, cmap='viridis', s=0.6*Parameters.Mesh_rad)
    plt.colorbar(sc, ax=ax, label='Value')
    plt.gca().set_aspect('equal', adjustable='box')
    plt.xlim(-Parameters.Mesh_rad*1.1, Parameters.Mesh_rad*1.1)
    plt.ylim(-Parameters.Mesh_rad*1.1, Parameters.Mesh_rad*1.1)
...
def plot_RSRP_from_HAPS(W: np.array, HAPS_channels, points, HAPS_users, Null_points, user=-1, save=''):
    ...
    y = np.array(y)
    value = RSRP_dbm
    value_sup = np.where(RSRP_dbm>-100, RSRP_dbm, -100)
    ...
    if save != '':
        plt.savefig(save)
    plt.show()
    print("RSRP at HAPS users : ", value[HAPS_users])
    print("RSRP at Null points : ", value[Null_points])

    fig= plt.figure(figsize=(8, 8))
    ax = plt.subplot()
    sc = plt.scatter(x, y, c=value_sup, cmap='viridis', s=0.6*Parameters.Mesh_rad)
    plt.colorbar(sc, ax=ax, label='Value')
    plt.gca().set_aspect('equal', adjustable='box')
    plt.xlim(-Parameters.Mesh_rad*1.1, Parameters.Mesh_rad*1.1)
    plt.ylim(-Parameters.Mesh_rad*1.1, Parameters.Mesh_rad*1.1)
